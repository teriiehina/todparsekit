...
#import "TODJsonParserTest.h"
#import "TODJsonParser.h"
#import "TODFastJsonParser.h"

@implementation TODJsonParserTest

- (void)setUp {
	p = [TODJsonParser parser];
}
...
- (void)testForAppleBossResultTokenization {
	...
	TODTokenizer *t = [[[TODTokenizer alloc] init] autorelease];
	t.string = s;
...
- (void)testCrunchBaseJsonParser {
	...
	s = [NSString stringWithContentsOfFile:path];
	TODJsonParser *parser = [[[TODJsonParser alloc] init] autorelease];
...
- (void)testCrunchBaseJsonTokenParser {
	NSString *path = [[NSBundle bundleForClass:[self class]] pathForResource:@"yahoo" ofType:@"json"];
	s = [NSString stringWithContentsOfFile:path];
	TODFastJsonParser *parser = [[[TODFastJsonParser alloc] init] autorelease];
	id res = [parser parse:s];
	res = res;
	//NSLog(@"res %@", res);
}
...
- (void)testYahoo4 {
	...
	p = [[[TODFastJsonParser alloc] init] autorelease];
	result = [p parse:s];
	//NSLog(@"result %@", result);
